...
misund = ['Что вы имели ввиду?', 'Не понял, повторите:', 'Пожалуйста, не перепутайте:', 'Выберите нужный вариант:', 'Было бы лучше, если бы вы выбрали один из вариатов:', 'Я не понимаю, о чем вы?', '⬇ Выбери один из вариантов ответов ⬇']
first_decoration = ['Да, слушаю 🙂', 'Чем могу помочь? 😃', 'Выбирайте:', 'Что рассказать?', 'Какой вопрос вас интересует?']
second_decoration = ['О чем вы хотите узнать больше?', 'Вот что я могу рассказать', 'Поясню:', 'Чем вы увлекаетесь?', 'Что из этого вас интересует?']
notification_student = ['Администрация скоро рассмотрит ваш вопрос 😌', 'Не переживайте, вам скоро ответят 🙂']
notification_ad = ['Мы обязательно рассмотрим ваше предложение в ближайшее время']
answers = {}
answers[frozenset({'справку', 'заказать'})] = 'Для этого есть деканат - закажите справку там, позвонив по телефону, который можете найти на нашем сайте: https://vsuet.ru/sveden/struct'
answers[frozenset({'справку', 'достать'})] = 'Для этого есть деканат - закажите справку там, позвонив по телефону, который можете найти на нашем сайте: https://vsuet.ru/sveden/struct'
answers[frozenset({'справку', 'взять'})] = 'Для этого есть деканат - закажите справку там, позвонив по телефону, который можете найти на нашем сайте: https://vsuet.ru/sveden/struct'
...
f_0 = Folder("Кем стать❓", "Выберите свое профессиональное направление:\n")
f_1 = Folder("Какие ЕГЭ сдавать❓", "Один предмет. Какой?\n")
f_2 = Folder("Подробнее о направлениях подготовки", "Какое направление подготовки вас интерисует?\n")
...
# Фразы
sw_notification_1 = 'Включен режим для абитуриента!'
sw_notification_2 = 'Ну, где там ваш вопрос? 🧐'
sw_notification_3 = 'Мы ждем :)'
sw_notification_4 = 'Жмите на кнопку! c:'
sw_notification_5 = 'Вы от куда?'

alert_get = 'Недостаточно данных для получения расписания. Вы можете добавить информацию о себе, воспользовавшись особым разделом ("Смена персональной информации")'
alert_faculty = 'Увы, такого факультета нет в моем списке. Пожалуйста, пользуйтесь кнопками'
alert_group = 'Такой подгруппы не может быть! Должно быть, вы ошиблись'

settings_step_0 = 'Для начала следует определится с факультетом. Выберите один из вариантов на клавиатуре'
settings_step_1 = 'Отлично! Теперь укажите вашу группу, например: "У-206"'
settings_step_2 = 'Параметр был сохранен. Осталось только узнать вашу подгруппу. Укажите "1"/"2" или, если у вас нет разделения, выберите любую из цифр'
settings_final = 'Все, настройки завершены. Пользуйтесь на здоровье 😉'
